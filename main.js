...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		Game.InjectMod=function(def)
		{
			//if (App) return false;
			if (localStorage.getItem("CookieClickerScript")) {
				Game.Prompt('<id InjectMod><h3>'+loc("Inject script")+'</h3><div class="block">'+loc("Please paste in the script to use to load the mod. This mod will be unloaded upon reloading! (Go to the Check Mod Data option to disable permanent mod loading. Because you have already imported a script, it will be overridden if you chose to load the mod every time; you may need to reload.)")+'<div id="scriptError" class="warning" style="font-weight:bold;font-size:11px;user-select:all;"></div></div><div class="block"><textarea id="textareaPrompt" style="width:100%;height:128px;">'+(def||'')+'</textarea></div>',[[loc("Load"),'if (l(\'textareaPrompt\').value.trim().length==0){return false;}try{Function(l(\'textareaPrompt\').value)();Game.ClosePrompt()}catch(e){l(\'scriptError\').textContent=loc("Error importing script (click to select text): ")+(e instanceof Error?e.stack||e.message:e+"");}'],[loc("Load every time"),'if (l(\'textareaPrompt\').value.trim().length==0){return false;}try{Game.permaLoad(l(\'textareaPrompt\').value);Game.ClosePrompt()}catch(e){l(\'scriptError\').textContent=loc("Error importing script (click to select text): ")+(e instanceof Error?e.stack||e.message:e+"");}'],[loc("Load autoclicker"),'l(\'textareaPrompt\').value=\'Game.LoadMod("autoclicker.js")\''],loc("Nevermind")]);
			} else {
				Game.Prompt('<id InjectMod><h3>'+loc("Inject script")+'</h3><div class="block">'+loc("Please paste in the script to use to load the mod. This mod will be unloaded upon reloading! (Go to the Check Mod Data option to disable permanent mod loading if you choose to load the mod every time.)")+'<div id="scriptError" class="warning" style="font-weight:bold;font-size:11px;user-select:all;"></div></div><div class="block"><textarea id="textareaPrompt" style="width:100%;height:128px;">'+(def||'')+'</textarea></div>',[[loc("Load"),'if (l(\'textareaPrompt\').value.trim().length==0){return false;}try{Function(l(\'textareaPrompt\').value)();Game.ClosePrompt()}catch(e){l(\'scriptError\').textContent=loc("Error importing script (click to select text): ")+(e instanceof Error?e.stack||e.message:e+"");}'],[loc("Load every time"),'if (l(\'textareaPrompt\').value.trim().length==0){return false;}try{Game.permaLoad(l(\'textareaPrompt\').value);Game.ClosePrompt()}catch(e){l(\'scriptError\').textContent=loc("Error importing script (click to select text): ")+(e instanceof Error?e.stack||e.message:e+"");}'],[loc("Load autoclicker"),'l(\'textareaPrompt\').value=\'Game.LoadMod("autoclicker.js")\''],loc("Nevermind")]);
			}
			l('textareaPrompt').focus();
		}
